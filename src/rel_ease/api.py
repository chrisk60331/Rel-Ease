...
class ReleaseAPI:
    ...
    def release(self, directory: str, version: str) -> dict[str, Any]:
        ...
        notes_body = self._release_notes(repo, request.version, status_before.get("files", []))
    ...
    def _release_notes(self, repo: RepoContext, version: str, status_files: list[dict[str, Any]]) -> str:
        changed = [item["path"] for item in status_files if not self._is_junk_or_secret(item["path"])]
        ...
        if changed:
            ...
            suffix = " and more" if len(changed) > 12 else ""
            lines.append(f"- Includes repository changes in {shown}{suffix}.")
        return "\n".join(lines)
